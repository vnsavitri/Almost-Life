...
export default function Loading() {
  ...
  useEffect(() => {
    ...
    // Build markers from all branches (up to 4), evenly spaced
    const pool = allBranches.length > 0 ? allBranches : [branch];
    const positions = [12, 34, 58, 80];
    const top: Marker[] = pool.slice(0, 4).map((b, i) => ({
      year: b.year,
      label: shortContext(b.context),
      pct: positions[i],
    }));
    const bottom: Marker[] = pool.slice(0, 4).map((b, i) => ({
      year: b.year,
      label: extractAlt(b.framing),
      pct: positions[i],
    }));
    setTopMarkers(top);
    setBottomMarkers(bottom);

    // Stagger marker reveal
    const timers: ReturnType<typeof setTimeout>[] = [];
    top.forEach((_, i) => timers.push(setTimeout(() => setVisibleTop((p) => [...p, i]), 1500 + i * 2000)));
    bottom.forEach((_, i) => timers.push(setTimeout(() => setVisibleBottom((p) => [...p, i]), 2200 + i * 2000)));
    ...
    // Kick off the real API call (only once)
    if (!calledRef.current) {
      ...
      fetch("/api/generate-life", {
        ...
        body: JSON.stringify({ branch, demo: isDemo || undefined, pdf_b64: pdfB64 || undefined }),
      })
        .then((r) => r.json())
        .then((data) => {
          if (data.life) {
            sessionStorage.setItem("almost_life", JSON.stringify(data.life));
          }
          navigate("/result");
        })
    ...
    // Safety timeout — navigate after 45s regardless
    const bail = setTimeout(() => navigate("/result"), 45000);
    ...
    <main style={{ backgroundColor: "#F5EFE6", color: "#1A1A1A" }} className="min-h-screen flex flex-col items-center justify-center px-8">
      ...
      <div style={{ width: "100%", maxWidth: "680px" }}>
        ...
        <div style={{ marginBottom: "5rem" }}>
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.6875rem", letterSpacing: "0.12em", textTransform: "uppercase", opacity: 0.35, marginBottom: "0.6rem" }}>
            this life
          </div>
            ...
            {topMarkers.map((m, i) =>
              visibleTop.includes(i) ? (
                <div key={i} className="marker-in" style={{ position: "absolute", left: `${m.pct}%`, top: "-30px", transform: "translateX(-50%)", textAlign: "center", opacity: 0 }}>
                  <div style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: "0.6rem", fontStyle: "italic", color: "#1A1A1A", opacity: 0.55, whiteSpace: "nowrap" }}>
                    {m.year}: {m.label}
                  </div>
        ...
        <div style={{ marginBottom: "5rem" }}>
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.6875rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "#E07856", opacity: 0.6, marginBottom: "0.6rem" }}>
            the other one
          </div>
          <div style={{ position: "relative", height: "24px" }}>
            ...
            {bottomMarkers.map((m, i) =>
              visibleBottom.includes(i) ? (
                <div key={i} className="marker-in" style={{ position: "absolute", left: `${m.pct}%`, bottom: "-30px", transform: "translateX(-50%)", textAlign: "center", opacity: 0 }}>
                  ...
                  <div style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: "0.6rem", fontStyle: "italic", color: "#E07856", opacity: 0.7, whiteSpace: "nowrap" }}>
                    {m.year}: {m.label}
                  </div>
                </div>
              ) : null
